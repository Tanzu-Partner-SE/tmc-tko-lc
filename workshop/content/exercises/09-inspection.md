...
For this exercise, download the Lite inspection result that was generated 
earlier.
...
```execute-1
tmc cluster inspection scan list --cluster-name {{ session_namespace }}-cluster -o json | jq '.scans[].status.conditions.READY'
```
